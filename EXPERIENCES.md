...
- 대량(1000만)의 원천 정보를 직접 화면에서 조회 및 수정을 원함
...
> maven, spring boot, ag-grid, jenkins, mybatis, jsp, oracle
> gemini-cli(3), claude code(sonnet 4.6)
...
- 5차 수정사항 개발
  - 작업개시허가 서류 제출시 사업장(site_cd) 선택 및 po 선택시 자동 입력
  - 작업개시허가 서류 제출 목록에 사업장(site_cd) 컬럼 추가
  - 작업개시허가 목록에 등록일자 컬럼 제거
  - 작업개시허가 목록에 사업장(site_cd) 컬럼 추가
  - 작업개시허가 목록에 엑셀 다운로드 기능 추가
  - 작업개시허가 상세에 각 문서의 승인여부 초기값을 승인으로 변경
  - 작업개시허가 서류 제출시 기존 선택한 사업장(site_cd) 또는 po 선택시 자동된 사업장 표시가 안되는 문제 수정
  - 작업개시허가 상세에 사업장(site_cd) 표시
- 6/04 요청 사항
  - 20260604-6 최초 입력시 추가발굴여부 1차 일자가 자동으로 입력되는 현상 과 '추가 발굴 여부' -> '중복 발굴 여부' 용어 표시 변경
  - 20260604-18 부적합 목록에서 데이터 수정할때 클릭만 해도 첫데이터 자동 입력되는 불편함 발생, 클릭 후 선택 가능 하도록 조정 필요
  - 20260604-19 부적합 현황 수기 입력 시 엑셀에 입력된 데이터 복사/붙여넣기 기능 추가
  - 20260604-14 레이아웃 위치 조회 팝업 레이아웃 위치 표시 도형 크기 축소
  - 20260604-8 공정 및 설비 마스터 목록 데이터를 복사/붙여넣기로 입력
  - 20260604-11 SHE 현장 부적합 관리 상세 위치 조회 버튼을 추가하여 클릭하면 최초 입력시 위치 선택으로 입력한 내용을 볼 수 있도록 함
  - 20260604-2 현장 부적합 관리 전체 사진 업로드 최대 1장에서 3장까지 업로드 증가
  - 20260604-4 공통 > 현장 Patrol > SHE 현장 부적합 관리 > 등록 or 상세 사고유형에 기타 항목 추가하여 기타 항목 선택 시 직접 텍스트 입력 기능 추가
  - 20260604-7 Master 관리 > 공정 및 설비 마스터 컬럼명 변경 공정 → 1차 구분, 설비코드 → 2차 구분
  - 20260604-10 공통 > 현장 Patrol > SHE 현장 부적합 관리 > 등록 or 상세 > 부적합 입력 검색 결과 목록에서 사진을 바로 볼수 있는 컬럼을 추가하여 사진을 볼 수 있는 기능 필요
  - 20260604-11 공통 > 현장 Patrol > SHE 현장 부적합 관리 > 등록 or 상세 > 부적합 입력 개선현황 작성시 개선 상세 목록의 개선비용 컬럼을 금액, 비고 두개의 컬럼으로 변경
  - 20260604-5 현장 부적합 관리 부적합 입력시 입력 목록 내 개선가이드 라인, 비고 컬럼 추가
  - 20260604-1 공통 > 현장 Patrol > SHE 현장 부적합 관리 개선결과 컬럼 의 데이터가 개선현황 마지막 차수에 해당하는 데이터가 나와야 함
- 6/17 요청 사항
  - 20260617-1 Master 관리 > 공정 및 설비 마스터 2 클릭, 수정진입 변경, 목록 데이터를 여러 행을 복사/붙여넣기로 입력
  - 20260617-6 Master 관리 > 공정 및 설비 마스터 행 추가 후 일부 삭제 하면 저장 선택하지 않는 행이 삭제되는 오동작
  - 20260617-2 Master 관리 > Site별 소속 마스터 2 클릭, 수정진입 변경목록 데이터를 여러 행을 복사/붙여넣기로 입력
  - 20260617-5 Master 관리 > Site별 소속 마스터 행 추가 후 일부 삭제 하면 저장 선택하지 않는 행이 삭제되는 오동작
  - 20260617-3 공통 > 현장 Patrol > SHE 현장 부적합 관리 > 등록 2 클릭, 수정진입 변경목록 데이터를 여러 행을 복사/붙여넣기로 입력
  - Master 관리 > Site별 소속 마스터 사용하지 않는 Site 를 숨길 수 있도록 변경 필요
